...
export const verifySmtpConnection = async (input: {
  smtpHost: string;
  smtpPort: number;
  smtpUser: string;
  smtpPass: string;
  ...
  if (!input.smtpHost || !input.smtpPort || !input.smtpUser || !input.smtpPass) {
    throw new EmailDeliveryError("SMTP_NOT_CONFIGURED", "SMTP is not configured for verification.");
  }
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log(`SMTP_VERIFY_OK host=${input.smtpHost} port=${String(input.smtpPort)} secure=${String(input.smtpSecure)}`);
    return {
      ok: true,
      verified: true,
      provider: "smtp"
    ...
    // eslint-disable-next-line no-console
    console.error(
      `SMTP_VERIFY_FAILED name=${smtpError.name} code=${smtpError.code} message=${smtpError.message} command=${
        smtpError.command || "(none)"
      } response=${smtpError.response || "(none)"}`
    );
    throw new EmailDeliveryError("SMTP_SEND_FAILED", smtpError.message, smtpError.code);
